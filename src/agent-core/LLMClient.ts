...
export function normalizeDecision(value: unknown): AgentDecision {
  ...
  const say = sanitizeSay(typeof obj.say === "string" ? obj.say : "");
...
export function fallbackDecision(prompt: AgentPrompt): AgentDecision {
  ...
  if (trigger.includes("冷场")) {
    return {
      say: gameSummary ? `我先看一眼局面，${gameSummary}，这把得稳一点。` : "直播间突然安静，我先偷偷把脑子开机一下。",
      emotion: "thinking",
      avatarAction: "think",
      shouldSpeak: true,
  ...
  if (trigger.includes("游戏状态")) {
    return {
      say: gameSummary ? `现在局面是${gameSummary}，我先按保守路线想一下。` : "我在看局面，先不急着下判断。",
      emotion: "focus",
      avatarAction: "think",
      shouldSpeak: true,
